 ...
 * @mainpage
 *
 *  这是Deal.II类和函数文档的主控文档。关于其他方面的文档，如系统构建，可以在其他地方找到。 此外, 
 *  <a href="Tutorial.html">该库的Tutorial教程可以链接到这里</a>.
 *
 *  Deal.II库中到许多类都可以分组为模块（请参考<a href="modules.html">Modules page</a>页面
 *  或者此页面顶部菜单栏中都相应条目），这些模块最终组合拼装为大型的有限元代码。下面给出的graph图，
 *  可以了解Deal.II主要类组如何进行组织交互，还可以单击进行互动（灰色框表示库的子集，灰色椭圆表示可
 *  选外部应用程序的子集）：
 *  
 *
 * @dot
